#include <src/libbpf.h> /* bpf_get_link_xdp_id + bpf_set_link_xdp_id */
...
int xdp_link_detach(int ifindex, __u32 xdp_flags, __u32 expected_prog_id)
{
	...
	if (!curr_prog_id) {
		fprintf(stderr, "INFO: %s() no curr XDP prog on ifindex:%d\n",
			       __func__, ifindex);
		return EXIT_OK;
	}
	...
	fprintf(stderr, "INFO: %s() removed XDP prog ID:%d on ifindex:%d\n",
